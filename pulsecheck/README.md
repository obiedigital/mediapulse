...
- **Real-time:** Socket.io, self-hosted via a custom Node server (`server.ts`) that wraps
  the Next.js request handler
...
npx prisma migrate deploy   # or `npx prisma migrate dev` on a fresh dev DB
npm run prisma:seed         # optional: creates a demo org, moderator, and session
npm run dev                 # starts the combined Next.js + Socket.io server on :3000
```
...
Production: `npm run build` then `npm start` (both go through `server.ts`, not
`next start`, since the Socket.io server needs to share the HTTP listener).
...
- `server.ts` runs one Node process serving both Next.js and a Socket.io server on the
  same port (`/socket.io`).
- Moderators and participants both join a `session:<id>` room on connect.
- The moderator paces the session: `activeSlideOrder` on the `sessions` row says which
  slide is currently live. Advancing it (`PATCH /api/sessions/:id`) broadcasts
  `session_state` to the room; participants re-render on that event, no polling.
- Every response POST broadcasts `response_added` (slide id + running count); the
  moderator's results view refetches that slide's aggregate on receipt. Aggregation
  itself (`src/lib/aggregate.ts`) runs server-side per request, including the
  demographic-segment filter, so the client never needs the raw response set.

## Low-bandwidth participant UI

The participant flow (`/j`, `/j/[sessionId]`) intentionally ships no charting library, no
animation library, and no rendering of aggregate results (that stays on the moderator's
big-screen view) — just the current slide and a form. The heaviest dependency it loads is
`socket.io-client`, needed for the moderator-paced model above.
